...
def process_data(old_date, task_queue):
    """
    ...
    while not task_queue.empty():
        ...
        try:
            ...
            df = ts.get_realtime_quotes(data)
            qfq_day = pd.DataFrame([[float(df.open), float(df.high), float(df.price), float(df.low), float(df.volume),
                                    float(df.amount), 0]], columns=['open', 'high', 'close', 'low', 'volume', 'amount',
                                                                    'factor'], index=[date_end])
            qfq_day = qfq_day.rename_axis("date")
            qfq_day.index = qfq_day.index.astype('datetime64[ns]')
            #time.sleep(1)
            # ts.get_h_data('002014', start=str('2017-12-1'), end=str('2017-12-15'), autype=None, drop_factor=False)
...
if __name__ == "__main__":
    ...
    process_data(old_date0, stock_code_queue)
