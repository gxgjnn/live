...
from vnTrader.vtGateway import *
...
class QuantOSTdApi(object):
    ...
    def subscribePositionSymbols(self):
        ...
        pf = data_process.get_holding_stock_detail_current()

        symbols = ''
        for code in list(pf.iloc[:, 1]):
            instcode = str(code)
            if str(code)[0] == '0':
                instcode += ".SZ"
            elif str(code)[0] == '6':
            ...
            d = ts.get_realtime_quotes(symbol)
            # quotes.items()
            ...
            tick.preClosePrice = float(d['preclose']) if 'preclose' in d else 0
            ...
            tick.bidPrice1 = float(d['b1_p'])
            tick.bidPrice2 = float(d['b2_p'])
            tick.bidPrice3 = float(d['b3_p'])
            tick.bidPrice4 = float(d['b4_p'])
            tick.bidPrice5 = float(d['b5_p'])

            tick.askPrice1 = float(d['a1_p'])
            tick.askPrice2 = float(d['a2_p'])
            tick.askPrice3 = float(d['a3_p'])
            tick.askPrice4 = float(d['a4_p'])
            tick.askPrice5 = float(d['a5_p'])

            tick.bidVolume1 = float(d['b1_v'])
            tick.bidVolume2 = float(d['b2_v'])
            tick.bidVolume3 = float(d['b3_v'])
            tick.bidVolume4 = float(d['b4_v'])
            tick.bidVolume5 = float(d['b5_v'])

            tick.askVolume1 = float(d['a1_v'])
            tick.askVolume2 = float(d['a2_v'])
            tick.askVolume3 = float(d['a3_v'])
            tick.askVolume4 = float(d['a4_v'])
            tick.askVolume5 = float(d['a5_v'])

            tick.upperLimit = float(d['limit_up']) if 'limit_up' in d else 0
            tick.lowerLimit = float(d['limit_down']) if 'limit_down' in d else 0
